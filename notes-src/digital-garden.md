---
id: digital-garden
title: digital garden
date: 2026-03-10
tags: [meta, garden, sse, tools]
links: [soft-stack-ecology, rhizome, zettelmind]
---

a [digital garden](https://notes.davidkopp.de/30-knowledge/digital-garden/) that is the gravitational centre of km.'s web presence. not a blog, not a portfolio, but something between a commonplace book, a field notebook, a zettelkasten, and a map.

built in pure html + css. no js. no analytics. no tracking. hosted on github pages, with assets on cloudflare r2.

the garden has five sections: weblog (loose writing), projects (things being built), clutter (art, poetry, zines), rhizome (this zettelkasten), and webroll (links worth keeping).

the whole thing is an expression of [[soft-stack-ecology]], built slowly, by hand, with care. [[zettelmind]] is the authoring layer; [[rhizome]] is where the notes surface publicly.

the garden is never finished. it composting and growing at the same time.